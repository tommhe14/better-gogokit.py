...
class PagedResource(Resource):
    def __init__(self, data, factory):
        ...
        self.items = [helper.object_to_dict(item) for item in data["_embedded"]["items"]]
        self.total_items = helper.object_to_dict(data["total_items"])
        self.page = helper.object_to_dict(data["page"])
        self.page_size = helper.object_to_dict(data["page_size"])
        
        for item in self.items:
            for link_key, link in item['_links'].items():
                if link_key == 'searchresult:category':
                    item['categoryid'] = int(link['href'].split('/')[-1])
                elif link_key == 'searchresult:event':
                    item['eventid'] = int(link['href'].split('/')[-1])
                elif link_key == 'event:listingconstraints':
                    item['listingconstraintid'] = int(link['href'].split('/')[-1])
...
class HalClient:
	...
	def get_resource(self, url, factory, params = None):
		response = factory(HttpClient.get(url, auth=HTTPBearerAuth(self.token_store.get_access_token()), params= params))
		helper = Helper() 
		return helper.object_to_dict(response)
